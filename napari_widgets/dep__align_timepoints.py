...
from imageUtils import find_most_likely_z_slice_using_CC,z_align_ragged_timecourse,z_translate_and_pad
    ...
    def widget(
        ...
        for t in progress(timepoints_to_register):
            ...
            if t == reference_timepoint:
                ...
                continue
            if path.exists(path.join(path.dirname(filelist.loc[t,'B']),'B_align.tif')) and not OVERWRITE:
                print(f'Skipping t = {t} because its R_shg_align.tif already exists')
                continue

            output_dir = path.dirname(filelist.loc[t,'B'])
            # Alignment code here
            print(f'\n ---- Working on t = {t} ----')
            #Load the target

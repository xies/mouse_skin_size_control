...
all_df['Cell cycle duration','Measurement time'] = np.nan
all_df['G1 duration','Measurement time'] = np.nan
all_df['SG2 duration','Measurement time'] = np.nan
for trackID,track in all_tracks.items():
    if track.iloc[0]['Complete cycle','Meta']:
        all_df.loc[zip(track['Frame'].values,track['TrackID'].values),
                   ('Cell cycle duration','Measurement time')] = len(track)*12
...
print(f'Number of 60h prior to divisions: {len(prev5_div_frame)}')
...
print(f'Number of 72h prior to divisions: {len(prev6_div_frame)}')
...
prev5_div_frame.to_pickle(path.join(dataset_dir,'divisions_60h.pkl'))
prev6_div_frame.to_pickle(path.join(dataset_dir,'divisions_72h.pkl'))
prev7_div_frame.to_pickle(path.join(dataset_dir,'divisions_84h.pkl'))
...
# features2drop_from_pca = ['X','Y','Z','X-pixels','Y-pixels']

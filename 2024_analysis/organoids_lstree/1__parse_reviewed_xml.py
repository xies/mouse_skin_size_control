...
filename = path.join(dirname,'dataset_deconv-mamut_reviewedbyMimi-mamut.xml')
...
birth_spots = pd.read_csv(path.join(dirname,'manual_cellcycle_annotations/division-Spot.csv'),skiprows=[1,2])
_spots.loc[np.isin(_spots['SpotID'],birth_spots['ID']),'Phase'] = 'Visible division'

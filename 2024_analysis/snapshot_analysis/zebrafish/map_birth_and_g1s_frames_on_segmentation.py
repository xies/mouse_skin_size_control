...
# Transfer the brith+G1S annotations
birth_spots = pd.read_csv(path.join(dirname,'Position001_Mastodon/birth/spots.csv'),skiprows=[1,2])
_spots['Birth'] = False
for _,spot in birth_spots.iterrows():
    _spots.loc[_spots['ID'] == spot['ID'],'Birth'] = True
    
g1s_spots = pd.read_csv(path.join(dirname,'Position001_Mastodon/g1s/spots.csv'),skiprows=[1,2])
_spots['G1S'] = False

...
T = transform.SimilarityTransform(translation=[135,70],rotation=np.deg2rad(2))
shifted_mCherry = rough_aligned_mCherry.copy()
shifted_venus = rough_aligned_venus.copy()
for t in tqdm(np.arange(target_T,len(files))):
...
refined_tmats = tmats + shifted_tmats
...
for t in tqdm(range(len(stacks_mCherry))):
    ...
    for z in range(stacks_mCherry[t].shape[0]):
        _stack_mCh[z,...] = transform.warp(stacks_mCherry[t][z,...],T)
        _stack_ven[z,...] = transform.warp(stacks_venus[t][z,...],T)

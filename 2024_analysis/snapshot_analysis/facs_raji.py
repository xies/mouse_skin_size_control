...
dirname = '/Users/xies/OneDrive - Stanford/In vitro/CV from snapshot/Flow/06-23-2024 Raji Hoechst EdU'
...
diploids.loc[:,'Log_EdU'] = np.log(diploids['RL1-A'])
pts = plt.scatter(diploids['VL1-A'],diploids['Log_EdU'],alpha=.005)

plt.xlabel('VL1-A');plt.ylabel('RL1-h');
selector = SelectFromCollection(plt.gca(), pts)
...
x = verts[:,0];y = verts[:,1]
p_ = Path(np.array([x,y]).T)
I = np.array([p_.contains_point([x,y]) for x,y in zip(diploids['VL1-A'],diploids['Log_EdU'])])

diploids.loc[I,'Phase'] = 'EdU_minus_2n'
...
diploids.loc[I,'Phase'] = 'EdU_minus_4n'
...
sb.barplot(diploids,y='FSC-A',x='Phase'
           ,estimator=stats.variation,errorbar=(lambda x: cvariation_ci(x,Nboot))
           ,order=['EdU_minus_2n','EdU_plus','EdU_minus_4n']
           )
plt.ylabel('CV of FSC')
# plt.ylim([0,.25])
plt.title('Raji, cell cycle determined by DAPI')
...
CV = pd.DataFrame()
CV['FSC'] = diploids.groupby('Phase')['FSC-A'].apply(stats.variation)
CV['SSC'] = diploids.groupby('Phase')['SSC-A'].apply(stats.variation)
CV['Rep'] = 2
